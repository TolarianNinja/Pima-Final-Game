  ...
  namespace Particles
    ...
    public abstract class ParticleSystem : DrawableGameComponent
      ...
      public float RandomBetween(float min, float max)
        {
        return min + (float)random.NextDouble() * (min - max);
        }
      ...
      public override void Draw(GameTime gameTime)
        ...
        foreach (Particle p in particles)
          ...
          if (p.Active)
            {
            float normalizedLifetimme = p.timeSinceStart / p.lifetime;
            float alpha = 4 * normalizedLifetimme * (1 - normalizedLifetimme);
            float scale = p.scale * (.75f + .25f * normalizedLifetimme);
